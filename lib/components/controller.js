...
var deviceWithEndpoints = function (self, nwkAddr, fullDev) {
    var deferred = Q.defer(),
        epQueries = [];
...
Controller.prototype.endDeviceAnnceHdlr = function (data) {
    ...
    this.once(joinEvent, function () {
     ...
     var start;
	if(dev){
		data.epList = dev.epList;

console.log(JSON.stringify(data))
	
		data.ieeeAddr = data.ieeeaddr;
		start = deviceWithEndpoints(self, data.nwkaddr,data);
	}else{
		start = this.simpleDescReq(data.nwkaddr, data.ieeeaddr).then(function (devInfo) {
        return devInfo;
    });
	}

    start.then(function (devInfo) {
        return devInfo;
    }).fail(function () {
		return self.simpleDescReq(data.nwkaddr, data.ieeeaddr);
    }).then(function (devInfo) {
console.log("after: " + JSON.stringify(devInfo.endpoints))
