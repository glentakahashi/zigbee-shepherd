...
Controller.prototype._endDeviceAnnceHdlr = function (data, aborted) {
    ...
    return Q(dev)
        ...
        if (_.isFunction(shepherd.acceptDevInterview)) {
            ...
            return shepherd.acceptDevInterview(info).timeout(6000).then(function(result){
                if(aborted()) return Q.reject("__abort__")
    ...
    .then(function(dev){
        ...
        if (dev && _.isFunction(shepherd.acceptDevIncoming)) {
            ...
            return shepherd.acceptDevIncoming(info).timeout(6000).then(function(result){
                if(result) return dev
    ...
    .then(function(dev){
        ...
        if(dev){
            ...
            return Q.fcall(function(){
                    return devbox.sync(dev._getId())
                        .fail(function(){
                            return devbox.set(dev._getId(), dev)
                                    .then(function(){
                                        return devbox.sync(dev._getId())
                                    })
                        })
            ...
            return devbox.sync(_dev._getId())
        }
    })
    .then(function () {
        /* Close timeout */
