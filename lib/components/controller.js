...
function Controller(shepherd, cfg) {
    ...
    this.on('ZDO:tcDeviceInd', function (tcData) {
        ...
        if (self._spinLock) {
            ...
            if(self._spinLock !== null) self._spinLock = true;
            self.endDeviceAnnceHdlr(data);
        }
    });
    ...
    this.on('ZDO:ieeeRsp', function(data){
        ...
        self._joinWaitList[data.ieeeaddr] = setTimeout(function(){
            self.endDeviceAnnceHdlr(data);
        }, 3000);
    });
}
...
Controller.prototype.deviceWithEndpoints = function (epList, nwkAddr) {
	...
	_.forEach(epList, function (epId) {
		var epQuery = self.querie.endpoint(nwkAddr, epId);
...
Controller.prototype.endDeviceAnnceHdlr = function (data) {
    ...
    if(!self._joinLocks[data.ieeeaddr]){
        self._joinLocks[data.ieeeaddr] = Q()
    }
    ...
    var ret = self._joinLocks[data.ieeeaddr].then(function(){
        debug.shepherd("** Joining %s (0x%s)", data.ieeeaddr, data.nwkaddr.toString(16))
        return self._endDeviceAnnceHdlr(data);
    })
    self._joinLocks[data.ieeeaddr] = ret.fail(function(){}).then(function(){debug.shepherd("** Done %s (0x%s)", data.ieeeaddr, data.nwkaddr.toString(16))})
    return ret;
}
...
Controller.prototype._endDeviceAnnceHdlr = function (data) {
    ...
    if (dev && dev.status === 'online' && dev.nwkAddr == data.nwkaddr){  // Device has already joined, do next item in queue
        if (self._joinQueue.length) {
            ...
            if (next) {
                ...
                if(self._spinLock !== null) self._spinLock = false;
            }
        } else {
            if(self._spinLock !== null) self._spinLock = false;
        }

        debug.shepherd("Device %s skipping join, already joined", dev.ieeeAddr)
    ...
    this.getShepherd().emit('joining', { type: 'associating', ieeeAddr: data.ieeeaddr, nwkAddr: data.nwkaddr }); 
    
    var start = null;
	if(dev){
		var devbox = this.getShepherd()._devbox;
		if(dev.endpoints && Object.keys(dev.endpoints).length != 0){
			if(dev.nwkAddr != data.nwkaddr){
				dev.nwkAddr = data.nwkaddr;
				start = Q.ninvoke(devbox, 'set', dev._getId(), dev).then(function (id) {
					return dev;
				});
			}else{
				start = Q.fcall(function(){
					return dev;
				});
			}
		}else if(dev.epList && dev.epList.length){
			var fullDev = {}
			fullDev.ieeeAddr = data.ieeeaddr;
			fullDev.nwkAddr = data.nwkaddr;
			fullDev.endpoints = {};
			if(data.capabilities) fullDev.capabilities = data.capabilities;
			
			start = self.deviceWithEndpoints(dev.epList, fullDev.nwkAddr).then(function (epInfos) {
    ...
    if(!start){
        start = this.simpleDescReq(data.nwkaddr, data.ieeeaddr)
            .then(function(devInfo){
                if(data.capabilities) devInfo.capabilities = data.capabilities;
                return devInfo
            })
    }
	
	var attemptLeave = function (err) {
		// Try and ask to rejoin		
		var error = function(){
		...
		return self.request('ZDO', 'mgmtLeaveReq', reqArgObj).fail(function(){
			error()
		}).then(function (rsp) {
			error();
		});
    ...
    return start
        ...
        .then(function (devInfo) {
            if(devInfo) {
                ...
                return attemptLeave("no devInfo")
            }
        }, attemptLeave)
        .then(function () {
            self.emit(joinEvent, '__timeout__');
