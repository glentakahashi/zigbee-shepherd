...
Controller.prototype.checkOnline = function (dev, callback) {
    ...
    return this.request('ZDO', 'nodeDescReq', { dstaddr: nwkAddr, nwkaddrofinterest: nwkAddr }).timeout(5000).fail(function (err) {
		console.log("["+ieeeAddr+" / "+nwkAddr+"] check online NDR fails (1): "+ err)
        return self.request('ZDO', 'nodeDescReq', { dstaddr: nwkAddr, nwkaddrofinterest: nwkAddr }).timeout(5000).fail(function (err) {
			console.log("["+ieeeAddr+" / "+nwkAddr+"] check online NDR fails (2): "+ err)
			return false;
		});
    }).then(function (r) {
		if(!r) return;
...
Controller.prototype.endDeviceAnnceHdlr = function (data) {
    ...
    var ret = self._joinLocks[data.ieeeaddr].then(function(){
        console.log("** Joining %s", data.ieeeaddr)
        return self._endDeviceAnnceHdlr(data);
    })
    self._joinLocks[data.ieeeaddr] = ret.fail(function(){}).then(function(){console.log("** Done %s", data.ieeeaddr)})
    return ret;
}
