...
Controller.prototype._endDeviceAnnceHdlr = function (data) {
    ...
    const _dev = dev
    return Q(dev).then(function(dev){
        /* Fill out endoints */
        if(!dev.incomplete) {
            for(var i in dev.endpoints){
                shepherd._attachZclMethods(dev.endpoints[i]);
            }
            return dev
        }
        ...
        return retry(function(){return self.simpleDescReq(dev.nwkAddr, dev.ieeeAddr)},2)
        ...
        try {
            ...
            return shepherd.af.zclFoundation(basicEpInst, basicEpInst, 0, 'read', [{ attrId: 4 }, { attrId: 5 }, { attrId: 7 }]).then(function (readStatusRecsRsp) {
                var data = {};
        ...
        _.forEach(dev.endpoints, function (ep) {
            clustersReqs.push(function () {
                return shepherd.af.zclClustersReq(ep, interviewEvents).then(function (clusters) {
