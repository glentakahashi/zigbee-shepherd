...
var deviceWithEndpoints = function (self, epList, fullDev) {
    var deferred = Q.defer(),
        epQueries = [];

	_.forEach(epList, function (epId) {
		var epQuery = self.querie.endpoint(fullDev.nwkAddr, epId);
		epQueries.push(epQuery);
	});

	return Q.all(epQueries).then(function (epInfos) {
...
Controller.prototype.endDeviceAnnceHdlr = function (data) {
    ...
    this.once(joinEvent, function () {
        ...
        if (self._joinQueue.length) {
            setImmediate(function () {
				var temp = self._joinQueue.shift();
				if(temp){
					temp();
				}
            });
	...
	if(dev){
		...
		if(dev.endpoints && Object.keys(dev.endpoints).length != 0){
			...
			start = deviceWithEndpoints(self, dev.epList, fullDev).then(function (devInfo) {				
    ...
    start.fail(function (err) {
		...
		if(devInfo) self.emit('ZDO:devIncoming', devInfo);
    }).done();
};
