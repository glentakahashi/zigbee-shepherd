...
var Q = require('q'),
    ...
    MutexPromise = require("mutex-promise");
...
Controller.prototype.endDeviceAnnceHdlr = function (data) {
    var self = this,
        mutex = new MutexPromise(data.ieeeaddr);
    ...
    return mutex.promise().then(function(mutex){
        mutex.lock();
...
Controller.prototype._endDeviceAnnceHdlr = function (data) {
    ...
    return start.fail(function (err) {
		return self.simpleDescReq(data.nwkaddr, data.ieeeaddr);
    }).fail(attemptLeave).then(function (devInfo) {
		if(devInfo) {
