...
Controller.prototype.reset = function (mode, callback) {
    ...
    return Q.fcall(function () {
        ...
        setTimeout(function(){
            ...
            this.removeListener('SYS:resetInd', resetListener);
        }, 1000)
...
Controller.prototype._indirect_send = function(dstAddr, _send){
    var sendTime, deferred = Q.defer()
    var self = this, afResendEvt = 'ZDO:networkStatus:' + dstAddr+':6'

    function handleResend(){
        if((Date.now() - sendTime) > 7000){
            debug.shepherd("possible indirect expiration, resending")
            send();
        }else{
    ...
    return {evt: afResendEvt, fn: handleResend, result: deferred.promise}
}

Controller.prototype.indirect_send = function(dstAddr, _send){
    var self = this
    var ret = this._indirect_send(dstAddr, _send).result

    ret.finally(function(){
        self.removeListener(indirecter.evt, indirecter.fn)
    });
