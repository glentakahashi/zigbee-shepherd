...
querie.device = function (ieeeAddr, nwkAddr, callback) {
    ...
    return Q(controller.limitConcurrency(
        ...
        return Q(controller.limitConcurrency(
            ()=>controller.request('ZDO', 'activeEpReq', { dstaddr: nwkAddr, nwkaddrofinterest: nwkAddr }),
            ieeeAddr
        )(true))
    }).then(function(rsp) {
...
querie.endpoint = function (ieeeAddr, nwkAddr, epId, callback) {
    proving.number(nwkAddr, 'nwkAddr should be a number.');

    return Q(controller.limitConcurrency(
        ()=>controller.request('ZDO', 'simpleDescReq', { dstaddr: nwkAddr, nwkaddrofinterest: nwkAddr, endpoint: epId }),
        ieeeAddr
    )(true)).then(function (rsp) {
...
querie.deviceWithEndpoints = function (nwkAddr, ieeeAddr, callback) {
    ...
    return querie.device(ieeeAddr, nwkAddr).then(function (devInfo) {
        fullDev = devInfo;
        var epInfos = [], epQueries = [];

        _.forEach(fullDev.epList, function (epId) {
            epQueries.push(function (epInfos) {
                return querie.endpoint(ieeeAddr, nwkAddr, epId).then(function (epInfo) {
                    epInfos.push(epInfo);
                    return epInfos;
                });
            });
