...
// }
    ...
    /* The CC2531 has limited resources and requires more throttling, the CC2538 is more powerful */
...
af.zclClusterAttrsReq = function (dstEp, cId, callback) {
    ...
    Q(af.limitConcurrency(()=>af.zclClusterAttrIdsReq(dstEp, cId))(true)).then(function (attrIds) {
        var readReq = [],
            attrsReqs = [],
            attributes = [],
        ...
        return Q.all(attrsReqs.map(af.limitConcurrency(c=>Q.fcall(c)))).then(function(){
            return attributes;
        });
