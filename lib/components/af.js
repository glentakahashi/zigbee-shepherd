...
// }
    ...
    const mappingFunc = function (cId) {
		...
		return af.zclClusterAttrsReq(dstEp, cId).then(handleAttributes).fail(function(err){
	...
	Q.fcall(mappingFunc(first))
        .then(function(){
            return Q.all(clusterList.map(mappingFunc))
        })
        .then(function(){
            var slowChain = Q(0)
            clusters.forEach(function(cluster, cIdString){
                if(cluster.error){
                    slowChain = slowChain.then(function(){
                        return af.zclClusterAttrsReq(dstEp, cluster.id).then(function(attrs){
                            cluster.attrs = attrs
                        }, function(err){
                            cluster.error = err
                        })
...
af.zclClusterAttrsReq = function (dstEp, cId, callback) {
    return Q(af.controller.limitConcurrency(()=>af.zclClusterAttrIdsReq(dstEp, cId))(true)).then(function (attrIds) {
        var readReq = [],
            attrsReqs = [],
            attributes = [],
        ...
        _.forEach(attrIds, function (id) {
            ...
            if (readReq.length === 5 || readReq.length === attrIdsLen) {
                ...
                attrsReqs.push(function () {
                    ...
                    return af.zclFoundation(dstEp, dstEp, cId, 'read', req)
                        .then(
                            ...
                            function(){
                                ...
                                req.forEach(function(r){
                                    singleChain = singleChain.then(function(){
                                        af.zclFoundation(dstEp, dstEp, cId, 'read', [r]).then(function (readStatusRecsRsp) {
                                            Array.prototype.push.apply(attributes,readStatusRecsRsp.payload);
                                        }).fail((err)=>{
                                            debug("An error occured when reading cluster: %s attr: %s. Error: %s", cId, JSON.stringify(r), err);
                                        });
                                    });
                                })
                                return singleChain						
        ...
        return Q.all(attrsReqs.map(af.controller.limitConcurrency(c=>Q.fcall(c)))).then(function(){
            return attributes;
        });
        ...
        case 'zclIncomingMsg':
            ...
            if (targetEp.isLocal()) {
                ...
                if (toLocalApp) {
                    ...
                    if(msg.zclMsg.cmdId === 'statusChangeNotification'&& msg.zclMsg.frameCntl.frameType == 1 && msg.zclMsg.payload)
