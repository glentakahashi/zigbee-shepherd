...
// }
    ...
    _.forEach(clusterList, function (cId) {
        ...
        clusterAttrsReqs.push(function () {
            return af.zclClusterAttrsReq(dstEp, cId).then(function (attrs) {
                ...
                return clusters;
            }).fail(function (err) { return clusters; });
        });
    });

    // all clusters
    var allReqs = Promise.all(clusterAttrsReqs.map(throat(4, c=>c())));


    /* The CC2531 has limited resources and requires throttling, the CC2538 is more powerful */
...
af.zclClusterAttrsReq = function (dstEp, cId, callback) {
    ...
    af.zclClusterAttrIdsReq(dstEp, cId).then(function (attrIds) {
        ...
        return Promise.all(attrsReqs.map(throat(4, c=>c()))).then(function(){
            return attributes;
        });
