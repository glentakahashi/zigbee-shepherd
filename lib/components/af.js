...
// }
	...
	Q.fcall(mappingFunc(first))
        ...
        .then(function(){
            var slowChain = Q(0)
            for(var cIdString in clusters){
                let cluster = clusters[cIdString]
                if(cluster.error){
...
af.zclClusterAttrsReq = function (dstEp, cId, callback) {
    ...
    Q(af.controller.limitConcurrency(()=>af.zclClusterAttrIdsReq(dstEp, cId))(true)).then(function (attrIds) {
        var readReq = [],
            attrsReqs = [],
            attributes = [],
                ...
                attrsReqs.push(function () {
                    return af.zclFoundation(dstEp, dstEp, cId, 'read', req).then(function (readStatusRecsRsp) {
						...
						return af.zclFoundation(dstEp, dstEp, cId, 'read', req).then(function (readStatusRecsRsp) {
							...
							debug("An error occured when reading cluster values: " + err);
						});
					});
                });
                attrIdsLen -= 5;
