...
handlers.devIncoming = function (devInfo, resolve) {
    ...
    var processDev = Q.fcall(function () {
        ...
        if(dev.building){
            return self._registerDev(dev).then(function(){return dev})
        }
        return dev;
    }).then(function(dev) {
    ...
    .then(function (dev) {
        ...
        self.remove(dev.getIeeeAddr(), { reJoin: true }, function (err) {
            if(err){
                self.remove(dev.getIeeeAddr(), { reJoin: false }, function (err) {
                    Q.ninvoke(self._devbox, 'remove', dev._getId());
                });
            }else{
                Q.ninvoke(self._devbox, 'remove', dev._getId());
