...
handlers.devIncoming = function (devInfo, resolve) {
    ...
    .then(function (dev) {
        console.log("ffalse %s" ,dev)
        if(!dev) return dev

        const checkClusters = Object.values(dev.endpoints)
                               .some(function(ep){return Object.keys(ep.clusters).length > 1});
        ...
        if (accepted) {
            ...
            self.remove(dev.getIeeeAddr(), { reJoin: true }).then(function () {
