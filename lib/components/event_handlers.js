...
handlers.devIncoming = function (devInfo, resolve) {
    ...
    function syncEndpoints(dev) {
		var endpoints = devInfo.endpoints;
		if(endpoints.forEach){
			...
			for(var i in endpoints){
				self._attachZclMethods(endpoints[i]);
			}
    ...
    var isFullEps = false, isRegistered = false;
    var processDev = Q.fcall(function () {
        ...
        try {
            ...
            return self.af.zclFoundation(basicEpInst, basicEpInst, 0, 'read', [{ attrId: 4 }, { attrId: 5 }, { attrId: 7 }]).then(function (readStatusRecsRsp) {
                ...
                throw new Error("Unable to query manditory cluster genBasic, error: %s", err);
            });
        } catch (err) {
            return dev;
        }
    ...
    .then(function (dev) {
        if (_.isFunction(self.acceptDevInterview)) {
            var info = {
            ...
            return Q.ninvoke(self, 'acceptDevInterview', info).timeout(60000).then(function(result){
                if(result) return dev
                throw new Error("not accepted during interview")
            })
        }
    ...
    .then(function (dev) {
        const checkClusters = Object.values(dev.endpoints).every(function(ep){return Object.keys(ep.clusters).length});
        if(isFullEps && checkClusters){
            debug.shepherd('Device: %s skipping interview.', dev.getIeeeAddr());
            return dev;
        }
        ...
        }, Q(0)).then(function(){return true});
    }).then(function (previous_accept) {
        if(!previous_accept) return false
        ...
        if (accepted) {
            var ret
            if(isRegistered){
                ret = Q.ninvoke(self._devbox, 'sync', dev._getId());
            }else{
                isRegistered = true
                ret = self._registerDev(dev)
            }
            ...
            debug.shepherd('Device: %s not accepted.', dev.getIeeeAddr())
            return self.remove(dev.getIeeeAddr(), { reJoin: true }).then(function () {
        ...
        if(!isRegistered){
            self._registerDev(dev)
        }
		self.remove(dev.getIeeeAddr(), { reJoin: true }).then(function (err) {
			if(err){
				self.remove(dev.getIeeeAddr(), { reJoin: true }).then(function (err) {
					Q.ninvoke(self._devbox, 'remove', dev._getId());
				});
