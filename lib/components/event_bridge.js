...
bridge._areqEventBridge = function (controller, msg) {
    ...
    if (msg.subsys === 'ZDO')
        ...
        bridge._nwkIndicationEventBridge(controller, msg);
...
bridge._nwkIndicationEventBridge = function (controller, msg) {
    var payload = msg.data,
        nwkEventHead = 'NWK:' + msg.ind,
        nwkBridgedEvent;

    switch (msg.ind) {
        case 'pollInd':
            nwkBridgedEvent = sapiEventHead
            break;
        default:  
            break;
    }
    ...
    if (nwkBridgedEvent)
        controller.emit(nwkBridgedEvent, payload);
};
