...
ZShepherd.prototype.list = function (ieeeAddrs) {
    ...
    if (_.isString(ieeeAddrs))
        ...
        ieeeAddrs = _.map(this._devbox.exportAllObjs().filter(function(a){return !a.incomplete}), function (dev) {
...
ZShepherd.prototype.lqiScan = function (ieeeAddr, done, emit) {
	var info = this.info();
	var self = this;
    var learn;
    const noDuplicate = {};
	
	var emitRecursive = function(ieeeAddr,nwkAddr,parent){
        ...
        if(dev) {
            if(emit){
                emit({nwkaddr: nwkAddr, ieeeaddr: ieeeAddr, status: dev.status, parent: parent, type: dev.type})
            }
            ...
            return Q.fcall(function(){
                try {
                    return self.lqi(ieeeAddr).fail(function(err){
                        ...
                        return learn(null, data, ieeeAddr);
                    });
                }catch(ex){
            ...
            emit({nwkaddr: nwkAddr, ieeeaddr: ieeeAddr, status: "offline", parent: parent, type: null})
        }
	}
	
	learn = function(err,data,parent){
        const queue = [];
		if(!err){
			for(var i=0;i<data.length;i++){
                ...
                const e = emitRecursive(ieeeAddr, data[i].nwkAddr, parent)
                if(e){
                    queue.push(e);
                }
			}
	...
	return learn(false, [{ieeeAddr: ieeeAddr, force: true}], null).then(function(){
        if(done) done(Object.values(noDuplicate))
    });
}
