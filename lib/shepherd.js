...
ZShepherd.prototype.reset = function (mode, callback) {
    ...
    if (mode === 'hard' || mode === 0) {
        ...
        if (self._devbox) {
            _.forEach(devbox.exportAllIds(), function (id) {
                removeDevs.push(devbox.remove(id));
            });
...
ZShepherd.prototype.mount = function (zApp, callback) {
    ...
    Q.fcall(function () {
        ...
        return self.controller.querie.coordInfo().then(function (coordInfo) {
            ...
            return self._devbox.sync(coord._getId());
        });
    }).then(function () {
...
ZShepherd.prototype.lqiScan = function (ieeeAddr, emit) {
	var info = this.info();
	var self = this;
    const noDuplicate = {};
        ...
        return function(data){
            ...
            for(var i=0;i<data.length;i++){
                ...
                if(dev && dev.type == "Router" && !noDuplicate[ieeeAddr]) {
                    chain = chain.then(function(){return self.lqi(ieeeAddr).then(processResponse(ieeeAddr), function(){})})
                }
...
ZShepherd.prototype._registerDev = function (dev, callback) {
    ...
    return Q.fcall(function () {
        if (oldDev) {
            ...
            return devbox.set(dev._getId(), dev).then(function (id) {
            ...
            return devbox.add(dev).then(function (id) {
                dev._setId(id);
                return id;
            });
        }
...
ZShepherd.prototype.clearDev = function (dev, callback) {
    return this._devbox.remove(dev).nodeify(callback);
};
