...
ZShepherd.prototype.lqiScan = function (done) {
	var info = this.info();
	var self = this;
    var learn;
    const noDuplicate = {};
	
	var emitRecursive = function(ieeeAddr,nwkAddr){
        if(ieeeAddr == '0x0000000000000000'){
            // Lost route
            return
        }
        var dev = self._findDevByAddr(ieeeAddr);
        if(dev) {
            if(dev.status === 'offline'){
                self.controller.emit('ZDO:endDeviceAnnceInd', { srcaddr: nwkAddr, nwkaddr: nwkAddr, ieeeaddr: ieeeAddr, capabilities: {} });
            }else{
                debug.shepherd("Device %s is %s and hence no announcement necessary", ieeeAddr, dev.status)
            }
            
            if(dev.type == "EndDevice") return;

            return Q.fcall(function(){
                try {
                    return self.lqi(ieeeAddr).fail(function(err){
                        return learn(err);
                    }).then(function(data){
                        return learn(null, data);
                    });
                }catch(ex){
            ...
            /* We must have fogrotten the interview results, disk corruption? */
            /* Will LQI later, can't LQI an unnknown device due to API limitations */
            self.controller.emit('ZDO:endDeviceAnnceInd', { srcaddr: nwkAddr, nwkaddr: nwkAddr, ieeeaddr: ieeeAddr, capabilities: {} });
        }
	}
	
	learn = function(err,data){
        const queue = [];
		if(!err){
			for(var i=0;i<data.length;i++){
				const ieeeAddr = data[i].ieeeAddr;
				
				if(ieeeAddr == info.net.ieeeAddr && !data[i].force){
					continue;
				}
                
                /* In case of recursion between routers */
                if(noDuplicate[ieeeAddr]){
                    continue;
                }
                noDuplicate[ieeeAddr] = data[i];
                
                const e = emitRecursive(ieeeAddr, data[i].nwkAddr)
                if(e){
                    queue.push(e);
                }
			}
	...
	return learn(false, [{ieeeAddr: info.net.ieeeAddr, force: true}]).then(function(){
        done(Object.values(noDuplicate))
    });
}
