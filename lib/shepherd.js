...
ZShepherd.prototype.lqiScan = function (done) {
	...
	var emitRecursive = function(device,ieeeAddr,nwkAddr){
		if(ieeeAddr != info.net.ieeeAddr){
			//self.emit('ind', { type: 'lqiFound', data: ieeeAddr, endpoints: endpoints});
			console.log("requesting simple desc")
			
			var dev = self._findDevByAddr(ieeeAddr);
			if(dev){
				if (dev.status === 'offline'){
					console.log('$$$$ LQI DEV: '+ieeeAddr);
	...
	learn = function(err,data){
		if(!err){
			for(var i=0;i<data.length;i++){
				...
				emitRecursive(device, ieeeAddr, data[i].nwkAddr);
			}
		}
	...
	learn(false, [{ieeeAddr: info.net.ieeeAddr, force: true}]);
}
