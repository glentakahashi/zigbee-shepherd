        ...
        return function(data){
            ...
            for(var i=0;i<data.length;i++){
                ...
                if(dev && dev.type == "Router" && !noDuplicate[ieeeAddr]) {
                    set[ieeeAddr] = self.lqi(ieeeAddr)
                }
                noDuplicate[ieeeAddr] = data[i]
            }
            /* Breadth first */
            return Q.all(Object.values(set)).then(function(){
                for(var ieeeAddr in set){
                    set[ieeeAddr] = set[ieeeAddr].then(processResponse(ieeeAddr), function(err){
